<x-main-layout>
    <div class="m-2 p-2 flex justify-between">
        <h3 class="mb-4 text-2xl font-bold text-black dark:text-white">{{ $event->title }}</h3>
        <div class="flex space-x-2 text-indigo-700 dark:text-white">
            From:
    ...
    <div class="mb-16 flex flex-wrap">
            ...
            <div class="flex flex-col">
                <div class="ripple relative overflow-hidden rounded-lg bg-cover bg-[50%] bg-no-repeat shadow-lg dark:shadow-black/20"
                    ...
                    <div class="flex space-x-2 p-4" x-data="{
                        ...
                        onHandleLike() {
                            axios.post(`/event-like/{{ $event->id }}`).then(res => {
                                this.eventLike = res.data
                            })                    
                        },
                        onHandleSavedEvent() {
                            axios.post(`/event-save/{{ $event->id }}`).then(res => {
                                this.savedEvent = res.data
                            })                    
                        },
                        onHandleAttending() {
                            axios.post(`/event-attending/{{ $event->id }}`).then(res => {
                        ...
                        <button type="button" @click="onHandleLike" 
                            ...
                            :class="eventLike ? 'bg-blue-700 hover:bg-blue-800' : 'bg-slate-400 hover:bg-slate-500'  ">
                        ...
                        class="text-white  focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2"
                        :class="savedEvent ? 'bg-orange-700 hover:bg-orange-800' : 'bg-slate-400 hover:bg-slate-500'  ">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-3.5 h3.5 mr-2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        class="text-white  focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2"
                        :class="attending ? 'bg-green-700 hover:bg-green-800' : 'bg-slate-400 hover:bg-slate-500'  ">
                        Attending
                ...
                <div class="flex flex-col p-4">
                    <span class="text-indigo-600  dark:text-yellow-600 font-semibold">Host Info</span>
                    <div class="flex space-x-4 mt-6 bg-slate-50 dark:bg-slate-900 p-2 rounded-md">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor" class="w-12 h-12 dark:text-white">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg></span>
                        <div class="flex flex-col">
                            <span class="text-xl  dark:text-white">{{ $event->user->name }}</span>
                            <span class="text-xl dark:text-white">{{ $event->user->email }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full shrink-0 grow-0 lg:w-6/12 lg:pl-6 bg-slate-50  dark:bg-slate-900 rounded-md p-2">
            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                Start: {{ $event->toArray()['start_date'] }} at <time> {{ $event->start_time }}</time>
            </p>
            <p>
            ...
            @auth
            <div
                class="container d-flex justify-content-center align-items-center w-50 mt-6 bg-slate-200 dark:bg-slate-800 p-4 rounded-md">
                <div class="">
                    <form action="{{ route('event.comment', $event->id) }}" class="flex justify-between space-x-2"
                        method="POST">
                        @csrf
                        <input type="text"
             ...
             <div class="w-full">
                    @foreach ($event->comments()->latest()->get() as $comment)
                        <div class="w-full p-4 duration-500">
                            <div class="flex items-center rounded-lg bg-white dark:bg-slate-700 p-4 shadow-md">
                                <div>
                                    ...
                                    @can('view' ,$comment)
                                    <form action="{{ route('event.comment.destroy', [$event->id, $comment->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button
